...
First we need a tool to encode a surface weather field
  ...
  as an image. [Script](./make.3var.plot.R)

Then we need a set of pairs of such images - a source image, and a target image from 6-hours later. Each pair should be separated by at least 5 days, so they are independent states. [Script](./make.training.batch.R)
