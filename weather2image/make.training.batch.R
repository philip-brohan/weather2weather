...
count<-0
source.dir<-sprintf("%s/weather2weather/data_as_images/source",Sys.getenv('SCRATCH'))
if(!file.exists(source.dir)) dir.create(source.dir,recursive=TRUE)
target.dir<-sprintf("%s/weather2weather/data_as_images/target",Sys.getenv('SCRATCH'))
